...
const MatchCard = ({ match }: Props) => {
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-6 flex items-center gap-4 hover:border-accent/30 transition-all">
      ...
      <div className="text-center min-w-[100px]">
        <div className="flex items-center justify-center gap-2">
          <span className={`text-3xl font-black ${piratesWon ? "text-accent" : "text-foreground"}`}>{match.pirates_score}</span>
          <span className="text-muted-foreground text-lg">-</span>
          <span className={`text-3xl font-black ${!piratesWon ? "text-accent" : "text-foreground"}`}>{match.opponent_score}</span>
        </div>
